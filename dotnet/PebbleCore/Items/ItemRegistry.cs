...
// Ported from Sources/PebbleCore/Items/ItemRegistry.swift.
//
// Defines static class `ItemRegistryGlobals` (module-level state + free funcs):
//   SHERDS, TRIM_PATTERNS, SPAWN_EGG_MOBS, registerAllItems().
//
// External symbols (defined elsewhere in the assembly, referenced by name):
//   registerItem, iid, itemDefs, FoodDef, ToolDef, ArmorDef  (Items/ItemDefs)
//   WOODS, COLORS                                             (World/BlockRegistry)
//   B, blockDefs, prettify                                   (World/BlockDefs → Reg)
...
public static class ItemRegistryGlobals
{
    public static readonly string[] SHERDS = { "angler", "archer", "arms_up", "blade", "brewer", "burn", "danger", "explorer", "friend",
                                               "heart", "heartbreak", "howl", "miner", "mourner", "plenty", "prize", "sheaf", "shelter", "skull", "snort" };
    public static readonly string[] TRIM_PATTERNS = { "sentry", "dune", "coast", "wild", "ward", "eye", "vex", "tide", "snout", "rib",
                                                      "spire", "wayfinder", "shaper", "silence", "raiser", "host" };
    public static readonly string[] SPAWN_EGG_MOBS = {
    ...
    {
        public int tier; public double speed; public int dur; public int ench;
        public double swordDmg; public double pickDmg; public double axeDmg; public double axeSpd;
        public double shovelDmg; public double hoeSpd;

        public TierSpec(int tier, double speed, int dur, int ench,
                        double swordDmg, double pickDmg, double axeDmg, double axeSpd,
                        double shovelDmg, double hoeSpd)
        {
            this.tier = tier; this.speed = speed; this.dur = dur; this.ench = ench;
            this.swordDmg = swordDmg; this.pickDmg = pickDmg; this.axeDmg = axeDmg; this.axeSpd = axeSpd;
            this.shovelDmg = shovelDmg; this.hoeSpd = hoeSpd;
        }
    }

    private struct ArmorMat
    {
        public string name; public int[] def; public double tough; public double kb; public int[] dur; public int ench;

        public ArmorMat(string name, int[] def, double tough, double kb, int[] dur, int ench)
        {
            this.name = name; this.def = def; this.tough = tough; this.kb = kb; this.dur = dur; this.ench = ench;
        }
    }
    ...
    public static void registerAllItems()
    {
        ...
        registered = true;
        if (blockDefs.Count == 0) throw new InvalidOperationException("registerAllBlocks() must run first");
        ...
        {
            skip.Add($"{w}_wall_sign"); skip.Add($"{w}_sign");
            skip.Add($"{w}_hanging_sign"); skip.Add($"{w}_door");
        }
        foreach (var c in COLORS) { skip.Add($"{c}_bed"); }
        skip.Add("flower_pot");

        string categoryForBlock(string name)
        {
        ...
        int burnTimeForBlock(string name)
        {
            ...
            if (new[] { "crafting_table", "cartography_table", "fletching_table", "smithing_table", "loom", "composter", "barrel", "chest", "trapped_chest" }.Contains(name)) { return 300; }
            if (name == "dried_kelp_block") { return 4000; }
            if (matches(name, "ladder|bowl")) { return 300; }
            return 0;
        }
        double compostForBlock(string name)
        {
        ...
        foreach (var bdef in blockDefs)
        {
            if (skip.Contains(bdef.name)) { continue; }
            registerItem(bdef.name, display: bdef.displayName, block: (ushort)bdef.id,
        ...
        registerItem("glow_berries", block: B.cave_vines, food: new FoodDef(hunger: 2, saturation: 0.4), category: "food", icon: "glow_berries", compostChance: 0.3);
        registerItem("sweet_berries", block: B.sweet_berry_bush, food: new FoodDef(hunger: 2, saturation: 0.4), category: "food", icon: "sweet_berries", compostChance: 0.3);
        ...
        // ---- tools ----
        var tierOrder = new[] { "wooden", "stone", "iron", "golden", "diamond", "netherite" };
        var TIERS = new Dictionary<string, TierSpec> {
            ["wooden"] = new TierSpec(tier: 0, speed: 2, dur: 59, ench: 15, swordDmg: 3, pickDmg: 1, axeDmg: 6, axeSpd: 0.8, shovelDmg: 1.5, hoeSpd: 1),
            ["stone"] = new TierSpec(tier: 1, speed: 4, dur: 131, ench: 5, swordDmg: 4, pickDmg: 2, axeDmg: 8, axeSpd: 0.8, shovelDmg: 2.5, hoeSpd: 2),
            ["iron"] = new TierSpec(tier: 2, speed: 6, dur: 250, ench: 14, swordDmg: 5, pickDmg: 3, axeDmg: 8, axeSpd: 0.9, shovelDmg: 3.5, hoeSpd: 3),
            ["golden"] = new TierSpec(tier: 0, speed: 12, dur: 32, ench: 22, swordDmg: 3, pickDmg: 1, axeDmg: 6, axeSpd: 1.0, shovelDmg: 1.5, hoeSpd: 1),
            ["diamond"] = new TierSpec(tier: 3, speed: 8, dur: 1561, ench: 10, swordDmg: 6, pickDmg: 4, axeDmg: 8, axeSpd: 1.0, shovelDmg: 4.5, hoeSpd: 4),
            ["netherite"] = new TierSpec(tier: 4, speed: 9, dur: 2031, ench: 15, swordDmg: 7, pickDmg: 5, axeDmg: 9, axeSpd: 1.0, shovelDmg: 5.5, hoeSpd: 4),
        };
        foreach (var mat in tierOrder)
        {
            var t = TIERS[mat];
            var burn = mat == "wooden" ? 200 : 0;
            registerItem($"{mat}_sword", tool: new ToolDef("sword", tier: t.tier, speed: 1.5, attackDamage: t.swordDmg, attackSpeed: 1.6, durability: t.dur, enchantability: t.ench), category: "combat", burnTime: burn);
            registerItem($"{mat}_pickaxe", tool: new ToolDef("pickaxe", tier: t.tier, speed: t.speed, attackDamage: t.pickDmg, attackSpeed: 1.2, durability: t.dur, enchantability: t.ench), category: "tools", burnTime: burn);
            registerItem($"{mat}_axe", tool: new ToolDef("axe", tier: t.tier, speed: t.speed, attackDamage: t.axeDmg, attackSpeed: t.axeSpd, durability: t.dur, enchantability: t.ench), category: "tools", burnTime: burn);
            registerItem($"{mat}_shovel", tool: new ToolDef("shovel", tier: t.tier, speed: t.speed, attackDamage: t.shovelDmg, attackSpeed: 1, durability: t.dur, enchantability: t.ench), category: "tools", burnTime: burn);
            registerItem($"{mat}_hoe", tool: new ToolDef("hoe", tier: t.tier, speed: t.speed, attackDamage: 1, attackSpeed: t.hoeSpd, durability: t.dur, enchantability: t.ench), category: "tools", burnTime: burn);
        }
        registerItem("shears", tool: new ToolDef("shears", tier: 0, speed: 1.5, attackDamage: 0, attackSpeed: 4, durability: 238, enchantability: 1), category: "tools");
        registerItem("flint_and_steel", tool: new ToolDef("flint_and_steel", tier: 0, speed: 1, attackDamage: 0, attackSpeed: 4, durability: 64, enchantability: 1), category: "tools");
        registerItem("fishing_rod", tool: new ToolDef("fishing_rod", tier: 0, speed: 1, attackDamage: 0, attackSpeed: 4, durability: 64, enchantability: 1), category: "tools", burnTime: 300);
        registerItem("bow", tool: new ToolDef("bow", tier: 0, speed: 1, attackDamage: 0, attackSpeed: 4, durability: 384, enchantability: 1), category: "combat", burnTime: 300);
        registerItem("crossbow", tool: new ToolDef("crossbow", tier: 0, speed: 1, attackDamage: 0, attackSpeed: 4, durability: 465, enchantability: 1), category: "combat", burnTime: 300);
        registerItem("trident", tool: new ToolDef("trident", tier: 0, speed: 1, attackDamage: 8, attackSpeed: 1.1, durability: 250, enchantability: 1), category: "combat");
        registerItem("brush", tool: new ToolDef("brush", tier: 0, speed: 1, attackDamage: 0, attackSpeed: 4, durability: 64, enchantability: 1), category: "tools");

        // ---- armor ----
        var ARMORS = new ArmorMat[] {
            new ArmorMat(name: "leather", def: new[] { 1, 3, 2, 1 }, tough: 0, kb: 0, dur: new[] { 56, 81, 76, 66 }, ench: 15),
            new ArmorMat(name: "chainmail", def: new[] { 2, 5, 4, 1 }, tough: 0, kb: 0, dur: new[] { 166, 241, 226, 196 }, ench: 12),
            new ArmorMat(name: "iron", def: new[] { 2, 6, 5, 2 }, tough: 0, kb: 0, dur: new[] { 166, 241, 226, 196 }, ench: 9),
            new ArmorMat(name: "golden", def: new[] { 2, 5, 3, 1 }, tough: 0, kb: 0, dur: new[] { 78, 113, 106, 92 }, ench: 25),
            new ArmorMat(name: "diamond", def: new[] { 3, 8, 6, 3 }, tough: 2, kb: 0, dur: new[] { 364, 529, 496, 430 }, ench: 10),
            new ArmorMat(name: "netherite", def: new[] { 3, 8, 6, 3 }, tough: 3, kb: 0.1, dur: new[] { 408, 593, 556, 482 }, ench: 15),
        };
        var SLOT_NAMES = new[] { "helmet", "chestplate", "leggings", "boots" };
        foreach (var a in ARMORS)
        {
            for (int s = 0; s < 4; s++)
            {
                registerItem($"{a.name}_{SLOT_NAMES[s]}",
                             armor: new ArmorDef(slot: s, defense: a.def[s], toughness: a.tough, knockbackRes: a.kb, durability: a.dur[s], enchantability: a.ench, material: a.name),
                             category: "combat");
            }
        }
        registerItem("turtle_helmet", armor: new ArmorDef(slot: 0, defense: 2, toughness: 0, knockbackRes: 0, durability: 275, enchantability: 9, material: "turtle"), category: "combat");
        registerItem("elytra", armor: new ArmorDef(slot: 1, defense: 0, toughness: 0, knockbackRes: 0, durability: 432, enchantability: 1, material: "elytra"), category: "combat", rarity: 2);
        registerItem("shield", maxStack: 1, category: "combat", icon: "shield");

        // ---- food ----
        int food(string name, int hunger, double sat, string display = null, int? maxStack = null,
                 bool alwaysEat = false, bool meat = false, bool fast = false,
                 (string effect, int duration, int amplifier, double chance)[] effects = null,
                 int rarity = 0, double compostChance = 0)
        {
            effects ??= new (string effect, int duration, int amplifier, double chance)[] { };
            return registerItem(name, display: display, maxStack: maxStack,
                         food: new FoodDef(hunger: hunger, saturation: sat, alwaysEat: alwaysEat, meat: meat, fast: fast, effects: new List<(string effect, int duration, int amplifier, double chance)>(effects)),
                         category: "food", rarity: rarity, compostChance: compostChance);
        }
        food("apple", 4, 2.4, compostChance: 0.65);
        food("golden_apple", 4, 9.6, alwaysEat: true, effects: new[] { ("regeneration", 100, 1, 1.0), ("absorption", 2400, 0, 1.0) }, rarity: 1);
        food("enchanted_golden_apple", 4, 9.6, alwaysEat: true, effects: new[] { ("regeneration", 400, 1, 1.0), ("resistance", 6000, 0, 1.0), ("fire_resistance", 6000, 0, 1.0), ("absorption", 2400, 3, 1.0) }, rarity: 3);
        ...
        food("chicken", 2, 1.2, display: "Raw Chicken", meat: true, effects: new[] { ("hunger", 600, 0, 0.3) });
        ...
        food("pufferfish", 1, 0.2, effects: new[] { ("poison", 1200, 1, 1.0), ("hunger", 300, 2, 1.0), ("nausea", 300, 0, 1.0) });
        ...
        food("poisonous_potato", 2, 1.2, effects: new[] { ("poison", 100, 0, 0.6) });
        ...
        food("rotten_flesh", 4, 0.8, meat: true, effects: new[] { ("hunger", 600, 0, 0.8) });
        food("spider_eye", 2, 3.2, effects: new[] { ("poison", 100, 0, 1.0) });
        ...
        foreach (var n in new[] { "raw_iron", "iron_ingot", "iron_nugget", "raw_copper", "copper_ingot", "raw_gold", "gold_ingot", "gold_nugget",
        ...
        foreach (var n in new[] { "flint", "clay_ball", "brick", "nether_brick", "bone", "bone_meal", "leather", "rabbit_hide", "feather" })
        {
            registerItem(n, category: "ingredients");
        }
        ...
        foreach (var n in new[] { "ghast_tear", "slime_ball", "magma_cream", "fermented_spider_eye", "glistering_melon_slice",
        ...
        foreach (var n in new[] { "prismarine_shard", "prismarine_crystals", "ink_sac", "glow_ink_sac", "scute", "honeycomb" })
        {
            registerItem(n, category: "ingredients");
        }
        registerItem("wheat", category: "ingredients", compostChance: 0.65);
        foreach (var n in new[] { "sugar", "paper", "book" }) { registerItem(n, category: "ingredients"); }
        ...
        foreach (var n in new[] { "arrow", "spectral_arrow", "tipped_arrow" }) { registerItem(n, category: "ingredients"); }
        ...
        // dyes
        foreach (var c in COLORS) { registerItem($"{c}_dye", category: "ingredients"); }
        ...
        foreach (var n in new[] { "cod_bucket", "salmon_bucket", "pufferfish_bucket", "tropical_fish_bucket", "axolotl_bucket", "tadpole_bucket" })
        {
        ...
        foreach (var n in new[] { "leather_horse_armor", "iron_horse_armor", "golden_horse_armor", "diamond_horse_armor" })
        {
        ...
        foreach (var n in new[] { "minecart", "chest_minecart", "furnace_minecart", "hopper_minecart", "tnt_minecart" })
        {
            registerItem(n, maxStack: 1, category: "tools");
        }
        ...
        foreach (var s in SHERDS) { registerItem($"{s}_pottery_sherd", category: "ingredients", icon: "pottery_sherd", rarity: 1); }
        ...
        // spawn eggs
        foreach (var m in SPAWN_EGG_MOBS) { registerItem($"{m}_spawn_egg", category: "spawn_eggs", icon: "spawn_egg"); }
