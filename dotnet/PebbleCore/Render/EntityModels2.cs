...
//
// Ported from Sources/PebbleCore/Render/EntityModels2.swift.
// Defines static class: EntityModels2Globals
//   (module free funcs: M2, bipedModel, skeletonModel, villagerModel,
//    villagerModelPacked, registerAllModels, catModel, quadModel2 (public);
//    rpart/part/partList/box, registerModels2, hoglinPaint, guardianPaint,
//    squidParts (private)).
// Surface via: global using static PebbleCore.EntityModels2Globals;
//
// External symbols (defined elsewhere, referenced by name as in Swift):
//   MODELS, shadeColor (EntityModelsGlobals); ModelBox/ModelPart/MobModel/
//   EntitySkin (EntityModels); detCos/detSin (DetMath).
//
// FILE-SPECIFIC: registerAllModels() is the entrypoint (called from
// EntityModels.ensureModels()); it registers the first batch then calls the
// private registerModels2().
...
public static class EntityModels2Globals
{
    public static void M2(string name, MobModel m) { MODELS[name] = m; }
    ...
    private static ModelPart partList(string name, (double, double, double) pivot, ModelBox[] boxes)
    {
        return new ModelPart(name: name, pivot: pivot, boxes: boxes);
    }
    ...
    // ---------------------------------------------------------------------------
    public static MobModel bipedModel(Action<EntitySkin> paint, double scale = 1, string anim = "biped",
                                      int texH = 64, string[] packTex = null)
    {
        packTex = packTex ?? new string[] { };
        return new MobModel(
            texW: 64, texH: texH,
            parts: new ModelPart[] {
    ...
    public static MobModel skeletonModel(Action<EntitySkin> paint, double scale = 1,
                                        string[] packTex = null)
    {
        packTex = packTex ?? new string[] { };
        return new MobModel(
    ...
    public static MobModel villagerModelPacked(string[] packTex, double scale, Action<EntitySkin> paint)
    {
        return villagerModel(paint, scale, packTex: packTex);
    }

    public static MobModel villagerModelPacked(string[] packTex, Action<EntitySkin> paint)
    {
        return villagerModel(paint, 1, packTex: packTex);
    }

    public static MobModel villagerModel(Action<EntitySkin> paint, double scale = 1, string[] packTex = null)
    {
        packTex = packTex ?? new string[] { };
        return new MobModel(
            texW: 64, texH: 64,
    ...
    public static void registerAllModels()
    {
        ...
        }, packTex: new string[] { "entity/player/wide/steve.png" }));

        // VILLAGER-LIKE
        M2("villager", villagerModelPacked(new string[] { "entity/villager/villager.png" }, s => {
        ...
        var vilBase = MODELS["villager"];
        foreach (var prof in new string[] { "farmer", "fisherman", "shepherd", "fletcher", "librarian", "cartographer",
                                            "cleric", "armorer", "weaponsmith", "toolsmith", "butcher", "leatherworker",
                                            "mason", "nitwit" })
        {
            M2($"villager_{prof}", new MobModel(texW: vilBase.texW, texH: vilBase.texH, parts: vilBase.parts,
                                            anim: vilBase.anim, scale: vilBase.scale, paint: vilBase.paint,
                                            packTex: new string[] { "entity/villager/villager.png",
                                                      $"entity/villager/profession/{prof}.png" }));
        }
        M2("wandering_trader", villagerModelPacked(new string[] { "entity/wandering_trader.png" }, s => {
            s.box(0, 0, 8, 10, 8, 0xb88a68);
        ...
        M2("zombie_villager", villagerModel(s => {
        ...
        }, 1, packTex: new string[] { "entity/zombie_villager/zombie_villager.png" }));

        // UNDEAD
        M2("zombie", bipedModel(s => {
        ...
        }, anim: "zombie", packTex: new string[] { "entity/zombie/zombie.png" }));
        M2("husk", bipedModel(s => {
        ...
        }, anim: "zombie", packTex: new string[] { "entity/zombie/husk.png" }));
        M2("drowned", bipedModel(s => {
        ...
        }, anim: "zombie", packTex: new string[] { "entity/zombie/drowned.png", "entity/zombie/drowned_outer_layer.png" }));
        M2("skeleton", skeletonModel(s => {
            ...
            for (int i = 0; i < 3; i++) { s.rect(20, 21 + i * 2, 8, 1, 0xc8c8c0); }
            s.box(40, 16, 2, 12, 2, 0xc8c8c0);
            s.box(0, 16, 2, 12, 2, 0xc8c8c0);
        }, packTex: new string[] { "entity/skeleton/skeleton.png" }));
        M2("stray", skeletonModel(s => {
            s.box(0, 0, 8, 8, 8, 0xb0c0c0);
            s.eyes(0, 0, 8, 1, 4, 5, 2, 2, 0x2c2c2c, 0x000000);
            s.box(16, 16, 8, 12, 4, 0x6a8a8a);
            s.box(40, 16, 2, 12, 2, 0xb0c0c0);
            s.box(0, 16, 2, 12, 2, 0xb0c0c0);
        }, packTex: new string[] { "entity/skeleton/stray.png", "entity/skeleton/stray_overlay.png" }));
        M2("wither_skeleton", skeletonModel(s => {
        ...
        }, scale: 1.2, packTex: new string[] { "entity/skeleton/wither_skeleton.png" }));

        // CREEPER
        M2("creeper", new MobModel(
            texW: 64, texH: 32,
            parts: new ModelPart[] {
            ...
            }, packTex: new string[] { "entity/creeper/creeper.png" }));

        // SPIDERS
        MobModel spiderModel(int skinC, int eyeC, double scale, string[] packTex = null)
        {
            packTex = packTex ?? new string[] { };
            ...
            for (int i = 0; i < 4; i++)
            {
                parts.Add(part($"legR{i}", (-4, 9, (double)(-4 + i * 3)), box(-14, -1, -1, 14, 2, 2, 18, 0)));
                parts.Add(part($"legL{i}", (4, 9, (double)(-4 + i * 3)), box(0, -1, -1, 14, 2, 2, 18, 0)));
            }
            return new MobModel(
                texW: 64, texH: 32, parts: parts.ToArray(), anim: "spider", scale: scale,
                paint: s => {
                    ...
                    // 8 eyes
                    int fu = 32 + 8, fv = 4 + 8;
                    foreach (var e in new (int, int, int)[] { (1, 2, 1), (6, 2, 1), (2, 4, 2), (5, 4, 2) })
                    {
                        int ex = e.Item1, ey = e.Item2, w2 = e.Item3;
                        s.rect(fu + ex, fv + ey, w2, 1, eyeC);
                    }
                }, packTex: packTex);
        }
        M2("spider", spiderModel(0x352a24, 0xc83a3a, 1, new string[] { "entity/spider/spider.png", "entity/spider/spider_eyes.png" }));
        M2("cave_spider", spiderModel(0x0c4c44, 0xc83a3a, 0.7, new string[] { "entity/spider/cave_spider.png", "entity/spider/spider_eyes.png" }));
        ...
        M2("goat", new MobModel(
            ...
            packTex: new string[] { "entity/goat/goat.png" }));

        // WOLF / CAT / FOX
        M2("wolf", new MobModel(
            texW: 64, texH: 32,
            parts: new ModelPart[] {
                part("head", (-1, 10.5, -7), box(-2, -3, -2, 6, 6, 4, 0, 0),
                     box(-2, 3, 0, 2, 2, 1, 16, 14), box(2, 3, 0, 2, 2, 1, 16, 14),
                     box(-0.5, -3, -5, 3, 3, 4, 0, 10)),
                rpart("body", (0, 10, 2), (-Math.PI / 2, 0, 0), box(-3, -7, -3, 6, 9, 6, 18, 14)),
                rpart("mane", (-1, 10, -3), (-Math.PI / 2, 0, 0), box(-3, -3, -3, 8, 6, 7, 21, 0)),
            ...
            packTex: new string[] { "entity/wolf/wolf.png" }));
        M2("cat", catModel(new string[] { "entity/cat/tabby.png" }));
        M2("fox", new MobModel(
            texW: 48, texH: 32,
            parts: new ModelPart[] {
                ...
                rpart("body", (0, 8, -6), (-Math.PI / 2, 0, 0), box(-3, -15, -3.5, 6, 11, 6, 24, 15)),
            ...
            packTex: new string[] { "entity/fox/fox.png" }));

        // HORSE FAMILY
        MobModel horseModel(int color, int maneC, double scale = 1, string anim = "horse", string[] packTex = null)
        {
            packTex = packTex ?? new string[] { };
            return new MobModel(
                texW: 64, texH: 64,
        ...
        M2("horse", horseModel(0x8a5c34, 0x3c2c1c, 1, "horse", new string[] { "entity/horse/horse_brown.png" }));
        M2("donkey", horseModel(0x8a7a6a, 0x5a4c40, 0.9, "horse", new string[] { "entity/horse/donkey.png" }));
        M2("mule", horseModel(0x6a4a32, 0x3a2a1c, 0.95, "horse", new string[] { "entity/horse/mule.png" }));
        M2("skeleton_horse", horseModel(0xc8c8c0, 0xb0b0a8, 1, "horse", new string[] { "entity/horse/horse_skeleton.png" }));
        ...
        M2("llama", new MobModel(
            ...
            packTex: new string[] { "entity/llama/creamy.png" }));
        // vanilla CamelModel geometry (128×128) — modern unrotated rig
        M2("camel", new MobModel(
            texW: 128, texH: 128,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/camel/camel.png" }));

        registerModels2();
    }

    public static MobModel catModel(string[] packTex)
    {
        return new MobModel(
            texW: 64, texH: 32,
            parts: new ModelPart[] {
                ...
                rpart("body", (0, 12, -10), (-Math.PI / 2, 0, 0), box(-2, -19, -8, 4, 16, 6, 20, 0)),
    ...
    private static void catPaint(EntitySkin s)
    {
        int fur = 0xc89858;
    ...
    public static MobModel quadModel2(
    ...
    // ===========================================================================
    private static void registerModels2()
    {
        // ENDERMAN
        M2("enderman", new MobModel(
            texW: 64, texH: 32,
            parts: new ModelPart[] {
            ...
            }, packTex: new string[] { "entity/enderman/enderman.png", "entity/enderman/enderman_eyes.png" }));
        M2("endermite", new MobModel(
            texW: 64, texH: 32,
            parts: new ModelPart[] {
            ...
            }, packTex: new string[] { "entity/endermite.png" }));
        M2("silverfish", new MobModel(
            texW: 64, texH: 32,
            parts: new ModelPart[] {
            ...
            }, packTex: new string[] { "entity/silverfish.png" }));
        M2("shulker", new MobModel(
            texW: 64, texH: 64,
            parts: new ModelPart[] {
            ...
            }, packTex: new string[] { "entity/shulker/shulker.png" }));
        // vanilla vex/allay rigs (32×32) — two-cube body, thin arms, wing planes
        void flyBiped(string name, double armW, ModelBox wingBox, double armRUV, double armLUV,
                      double scale, string[] packTex, Action<EntitySkin> paint)
        {
            M2(name, new MobModel(
                texW: 32, texH: 32,
                parts: new ModelPart[] {
        ...
        flyBiped("vex", 2, box(-8, -5, 0, 8, 5, 0, 16, 22), 23, 23, 0.5, new string[] { "entity/illager/vex.png" }, s => {
        ...
        flyBiped("allay", 1, box(0, -5, 0, 0, 5, 8, 16, 14), 23, 23, 0.45, new string[] { "entity/allay/allay.png" }, s => {
        ...
        // ILLAGERS
        MobModel illagerModel(int robe, int skinTone = 0x9a9a8a, string[] packTex = null)
        {
            packTex = packTex ?? new string[] { };
            return new MobModel(
                texW: 64, texH: 64,
        ...
        M2("pillager", illagerModel(0x4c4438, 0x9a9a8a, new string[] { "entity/illager/pillager.png" }));
        M2("vindicator", illagerModel(0x3c4448, 0x9a9a8a, new string[] { "entity/illager/vindicator.png" }));
        M2("evoker", illagerModel(0x3a3a44, 0xb8b8a8, new string[] { "entity/illager/evoker.png" }));
        // vanilla ravager rig (128×128) — rotated two-cube body, mouth, neck, horns
        M2("ravager", new MobModel(
            texW: 128, texH: 128,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/illager/ravager.png" }));
        M2("witch", ((Func<MobModel>)(() => {
            var bas = illagerModel(0x2c2438, 0xb89a8a);
            var parts = bas.parts.ToList();
            parts.Add(new ModelPart(name: "hat", pivot: (0, 34, 0), boxes: new ModelBox[] {
                box(-5, 0, -5, 10, 2, 10, 0, 64), box(-3.5, 2, -3, 7, 4, 7, 0, 76),
                box(-2, 6, -1.5, 4, 4, 4, 0, 87), box(-0.5, 10, 0, 1, 2, 1, 0, 95),
            }));
            return new MobModel(texW: 64, texH: 128, parts: parts.ToArray(), anim: "illager", scale: 1,
                            paint: s => {
                            ...
                            }, packTex: new string[] { "entity/witch.png" });
        }))());

        // GOLEMS
        M2("iron_golem", new MobModel(
            texW: 128, texH: 128,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/iron_golem/iron_golem.png" }));
        M2("snow_golem", new MobModel(
            texW: 64, texH: 64,
            parts: new ModelPart[] {
            ...
            }, packTex: new string[] { "entity/snow_golem.png" }));
        ...
        MobModel piglinModel(string[] packTex, int skin, int cloth)
        {
            return new MobModel(
                texW: 64, texH: 64,
                parts: new ModelPart[] {
        ...
        M2("piglin", piglinModel(new string[] { "entity/piglin/piglin.png" }, 0xe8a4a4, 0x5a4a3a));
        M2("piglin_brute", piglinModel(new string[] { "entity/piglin/piglin_brute.png" }, 0xe8a4a4, 0x3a3026));
        M2("zombified_piglin", piglinModel(new string[] { "entity/piglin/zombified_piglin.png" }, 0xd89494, 0x8a5c4c));
        // vanilla hoglin rig (128×64) — tusked head pitched 50°, maned body
        var hoglinModel = new MobModel(
            texW: 128, texH: 64,
            parts: new ModelPart[] {
            ...
            anim: "quad", scale: 1,
            paint: hoglinPaint, packTex: new string[] { "entity/hoglin/hoglin.png" });
        M2("hoglin", hoglinModel);

        M2("blaze", new MobModel(
            texW: 64, texH: 32,
            parts: ((Func<ModelPart[]>)(() => {
                var parts = new List<ModelPart> { part("head", (0, 12, 0), box(-4, 0, -4, 8, 8, 8, 0, 0)) };
                for (int i = 0; i < 12; i++)
                {
                    parts.Add(part($"rod{i}", (0, 11, 0), box(-1, -10, -1, 2, 8, 2, 0, 16)));
                }
            ...
            }, packTex: new string[] { "entity/blaze.png" }));
        M2("ghast", new MobModel(
            texW: 64, texH: 32,
            parts: ((Func<ModelPart[]>)(() => {
                // sized for render scale 4.5: body fills the 4-block hitbox from the feet
                var parts = new List<ModelPart> { part("body", (0, 0, 0), box(-8, 0, -8, 16, 16, 16, 0, 0)) };
                for (int i = 0; i < 9; i++)
                {
                    double x = (double)((i % 3) - 1) * 5;
                    double z = (double)((i / 3) - 1) * 5;
                    parts.Add(part($"tent{i}", (x, 1, z), box(-1, -9, -1, 2, 9, 2, 0, 0)));
                }
                return parts.ToArray();
            }))(),
            ...
            }, packTex: new string[] { "entity/ghast/ghast.png" }));
        // vanilla magma cube rig (64×32) — eight 1-tall slices + molten core
        M2("magma_cube", new MobModel(
            texW: 64, texH: 32,
            parts: new ModelPart[] {
                part("body", (0, 0, 0),
            ...
            paint: s => {
                ...
                foreach (int row in new int[] { 1, 5, 7 })
                {
                    for (int x = 0; x < 32; x++) { if (s.rand(x, row) < 0.6) { s.px(x, row, 0xe8521a); } }
                }
                s.fill(4, 16, 16, 8, 0xff7b1d, 0.25);
            },
            packTex: new string[] { "entity/slime/magmacube.png" }));
        M2("slime", new MobModel(
            texW: 64, texH: 32,
            parts: new ModelPart[] {
            ...
            }, packTex: new string[] { "entity/slime/slime.png" }));
        // vanilla strider rig (64×128) — body + six side bristle planes
        M2("strider", new MobModel(
            texW: 64, texH: 128,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/strider/strider.png" }));

        // WATER MOBS
        M2("squid", new MobModel(
            ...
            }, packTex: new string[] { "entity/squid/squid.png" }));
        M2("glow_squid", new MobModel(
            texW: 64, texH: 32, parts: squidParts(), anim: "squid", scale: 1,
            paint: s => {
                s.box(0, 0, 12, 16, 12, 0x2c8a8a, 0.12);
                for (int i = 0; i < 18; i++) { s.px((int)(s.rand(i, 0) * 48), (int)(s.rand(i, 1) * 24), 0x6ae8d8); }
        ...
        M2("cod", new MobModel(
            ...
            packTex: new string[] { "entity/fish/cod.png" }));
        M2("salmon", new MobModel(
            texW: 32, texH: 32,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/fish/salmon.png" }));
        ...
        M2("tropical_fish", new MobModel(
            ...
            packTex: new string[] { "entity/fish/tropical_a.png", "entity/fish/tropical_a_pattern_1.png" },
            packTexTints: new int[] { 0xF6603A, 0xF8F8F8 }));
        M2("pufferfish", new MobModel(
            texW: 32, texH: 32,
            parts: new ModelPart[] {
            ...
            paint: s => {
                ...
                for (int y = 16; y < 24; y++)
                {
                    for (int x = 14; x < 28; x++) { if (s.rand(x, y) < 0.25) { s.px(x, y, 0x4c4c34); } }
                }
            },
            packTex: new string[] { "entity/fish/pufferfish.png" }));
        // vanilla dolphin rig (64×64)
        M2("dolphin", new MobModel(
            texW: 64, texH: 64,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/dolphin.png" }));
        ...
        var guardianModel = new MobModel(
            texW: 64, texH: 64,
            parts: ((Func<ModelPart[]>)(() => {
                var parts = new List<ModelPart> {
                ...
                for (int i = 0; i < 12; i++) { parts.Add(part($"spike{i}", (0, 12, 0), box(-1, -4.5, -1, 2, 9, 2, 0, 0))); }
                return parts.ToArray();
            }))(),
            anim: "guardian", scale: 1,
            paint: guardianPaint, packTex: new string[] { "entity/guardian.png" });
        M2("guardian", guardianModel);
        // vanilla axolotl rig (64×64) — gill planes, top fin, flat paddle legs
        M2("axolotl", new MobModel(
            texW: 64, texH: 64,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/axolotl/axolotl_lucy.png" }));
        // vanilla turtle rig (128×64) — rotated shell + flat flippers
        M2("turtle", new MobModel(
            texW: 128, texH: 64,
            parts: new ModelPart[] {
                part("head", (0, 5, -10), box(-3, -4, -3, 6, 5, 6, 2, 0)),
                rpart("body", (0, 13, -10), (-Math.PI / 2, 0, 0),
            ...
            packTex: new string[] { "entity/turtle/big_sea_turtle.png" }));
        // vanilla frog rig (48×48) — body/head planes, eyes, webbed foot planes
        M2("frog", new MobModel(
            texW: 48, texH: 48,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/frog/temperate_frog.png" }));
        // vanilla tadpole rig (16×16)
        M2("tadpole", new MobModel(
            texW: 16, texH: 16,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/tadpole/tadpole.png" }));
        ...
        M2("bat", new MobModel(
            ...
            }, packTex: new string[] { "entity/bat.png" }));
        M2("bee", new MobModel(
            texW: 64, texH: 64,
            parts: new ModelPart[] {
            ...
            }, packTex: new string[] { "entity/bee/bee.png" }));
        // vanilla parrot rig (32×32)
        M2("parrot", new MobModel(
            texW: 32, texH: 32,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/parrot/parrot_red_blue.png" }));
        ...
        M2("panda", new MobModel(
            ...
            parts: new ModelPart[] {
                ...
                rpart("body", (0, 14, 0), (-Math.PI / 2, 0, 0),
        ...
        M2("polar_bear", new MobModel(
            ...
            packTex: new string[] { "entity/bear/polarbear.png" }));
        // vanilla sniffer rig (192×192) — six legs, beaked head with tall ears
        M2("sniffer", new MobModel(
            texW: 192, texH: 192,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/sniffer/sniffer.png" }));

        // WARDEN
        // vanilla warden rig (128×128) — ribcage + tendril planes included
        M2("warden", new MobModel(
            texW: 128, texH: 128,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/warden/warden.png" }));
        ...
        M2("ender_dragon", new MobModel(
            ...
            packTex: new string[] { "entity/enderdragon/dragon.png" }));
        M2("wither", new MobModel(
            texW: 64, texH: 64,
            parts: new ModelPart[] {
            ...
            }, packTex: new string[] { "entity/wither/wither.png" }));
        M2("phantom", new MobModel(
            texW: 64, texH: 64,
            parts: new ModelPart[] {
            ...
            }, packTex: new string[] { "entity/phantom.png" }));

        // projectile / object models
        M2("arrow_model", new MobModel(
            texW: 16, texH: 16,
            parts: new ModelPart[] { part("body", (0, 0, 0), box(-0.5, -0.5, -4, 1, 1, 8, 0, 0)) },
            anim: "none", scale: 1,
            paint: s => { s.box(0, 0, 1, 1, 8, 0x8a6a3c, 0.15); s.rect(1, 1, 6, 1, 0xc8c8c8); }));
        M2("item_holder", new MobModel(
            texW: 16, texH: 16,
            parts: new ModelPart[] { part("body", (0, 0, 0), box(-4, 0, -4, 8, 8, 8, 0, 0)) },
            anim: "none", scale: 1,
            paint: s => { s.fill(0, 0, 16, 16, 0xffffff, 0); }));
        ...
        M2("boat_model", new MobModel(
            texW: 128, texH: 64,
            parts: new ModelPart[] {
                rpart("bottom", (1, 1, 0), (Math.PI / 2, Math.PI / 2, 0),
                      box(-14, -9, -3, 28, 16, 3, 0, 0)),
                part("stern", (0, 0, 14), box(-9, 0, -1, 18, 6, 2, 0, 19)),
                rpart("bow", (0, 0, -14), (0, Math.PI, 0), box(-8, 0, -1, 16, 6, 2, 0, 27)),
                rpart("sideR", (-8, 0, 0), (0, -Math.PI / 2, 0), box(-14, 0, -1, 28, 6, 2, 0, 35)),
                rpart("sideL", (8, 0, 0), (0, Math.PI / 2, 0), box(-14, 0, -1, 28, 6, 2, 0, 43)),
            ...
            packTex: new string[] { "entity/boat/oak.png" }));
        ...
        M2("end_crystal_model", new MobModel(
            texW: 64, texH: 32,
            parts: new ModelPart[] {
            ...
            packTex: new string[] { "entity/end_crystal/end_crystal.png" }));
        // vanilla minecart rig (64×32) — rotated floor plate + four 16×8×2 walls
        M2("minecart_model", new MobModel(
            texW: 64, texH: 32,
            parts: new ModelPart[] {
                rpart("floor", (0, 2, 0), (-Math.PI / 2, 0, 0), box(-10, -8, -1, 20, 16, 2, 0, 10)),
                part("sideL", (0, 0, 7), box(-8, 0, -1, 16, 8, 2, 0, 0)),
                part("sideR", (0, 0, -7), box(-8, 0, -1, 16, 8, 2, 0, 0)),
                rpart("endF", (-9, 0, 0), (0, Math.PI / 2, 0), box(-8, 0, -1, 16, 8, 2, 0, 0)),
                rpart("endB", (9, 0, 0), (0, -Math.PI / 2, 0), box(-8, 0, -1, 16, 8, 2, 0, 0)),
            },
            ...
            packTex: new string[] { "entity/minecart.png" }));

        // clones that tweak a base model (ocelot, zoglin, elder_guardian, goat_kid)
        var cat = MODELS["cat"];
        M2("ocelot", new MobModel(texW: cat.texW, texH: cat.texH, parts: cat.parts, anim: cat.anim, scale: cat.scale,
                              paint: s => { catPaint(s); s.fill(20, 0, 4, 4, 0xc8a23c, 0.3); },
                              packTex: new string[] { "entity/cat/ocelot.png" }));
        var hog = MODELS["hoglin"];
        M2("zoglin", new MobModel(texW: hog.texW, texH: hog.texH, parts: hog.parts, anim: hog.anim, scale: hog.scale,
                              paint: s => { hoglinPaint(s); s.fill(0, 0, 23, 15, 0x7aa05c, 0.25); },
                              packTex: new string[] { "entity/hoglin/zoglin.png" }));
        var guardian = MODELS["guardian"];
        M2("elder_guardian", new MobModel(texW: guardian.texW, texH: guardian.texH, parts: guardian.parts, anim: guardian.anim, scale: 2.3,
                                      paint: s => { guardianPaint(s); s.fill(0, 0, 36, 12, 0xb8b8b0, 0.2); },
                                      packTex: new string[] { "entity/guardian_elder.png" }));
        var goat = MODELS["goat"];
        M2("goat_kid", new MobModel(texW: goat.texW, texH: goat.texH, parts: goat.parts, anim: goat.anim, scale: 0.5, paint: goat.paint));
    }

    private static void hoglinPaint(EntitySkin s)
    {
        int c = 0xb87a5c;
    ...
    private static void guardianPaint(EntitySkin s)
    {
        s.box(0, 0, 12, 12, 12, 0x5a7a6a, 0.14);
        // orange spots
        for (int i = 0; i < 12; i++) { s.px((int)(s.rand(i, 0) * 46), (int)(s.rand(i, 1) * 22), 0xc87a3c); }
    ...
    private static ModelPart[] squidParts()
    {
        var parts = new List<ModelPart> { part("body", (0, 14, 0), box(-6, -8, -6, 12, 16, 12, 0, 0)) };
        for (int i = 0; i < 8; i++)
        {
            double ang = (double)i / 8 * Math.PI * 2;
            parts.Add(part($"tent{i}", (detCos(ang) * 5, 7, detSin(ang) * 5), box(-1, -18, -1, 2, 18, 2, 48, 0)));
        }
        return parts.ToArray();
    }
}
